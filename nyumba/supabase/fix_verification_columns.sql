...
-- Fix existing rows with wrong 'verified' status (set to 'approved' which is what the code expects)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'dalali_profiles' AND policyname = 'admin_full_access_dalali_profiles'
  ) THEN
