...
export default function DalaliDetailClient({ dalali }: { dalali: DalaliDetail }) {
  ...
  // grace_period included — same "current subscription" definition used
  // everywhere else that gates a dalali's plan (subscriptions/can-post),
  // so this page's badge doesn't show "Free" for a dalali who's still
  // actually entitled during their grace period.
  const activeSub = dalali.subscriptions?.find(s => s.status === 'active' || s.status === 'grace_period')
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        {/* ── Payment history + fix tools ── */}
        <PaymentHistoryPanel userId={dalali.id} />
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
          ...
          <div className="space-y-2">
            ...
            {/* Panua/badilisha usajili */}
            <button
              onClick={() => setShowExtendModal(true)}
              className="w-full py-3 rounded-xl text-sm font-semibold bg-blue-50 text-blue-700 border border-blue-200 active:scale-[0.98] transition-all"
            >
              <i className="ti ti-calendar-plus" aria-hidden="true" /> Panua/Badilisha Usajili
            </button>
      ...
      {/* ── Extend/change subscription modal ── */}
      {showExtendModal && (
        <ExtendSubscriptionModal
          dalaliId={dalali.id}
          dalaliName={dalali.full_name}
          currentPlan={activeSub?.plan}
          onClose={() => setShowExtendModal(false)}
          onDone={msg => { setSuccess(msg); router.refresh() }}
        />
      )}
    </div>
  )
