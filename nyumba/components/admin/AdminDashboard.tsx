          ...
          <div className="space-y-2 mb-3">
            ...
            {v.business_license_url && (
              <button
                ...
                className="w-full flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2.5 text-xs font-semibold text-amber-700 active:scale-95 transition-all"
              >
                <i className="ti ti-file-type-pdf text-base text-red-500" aria-hidden="true" />
                <span className="flex-1 text-left">Fungua Leseni ya Udalali (PDF)</span>
                <i className="ti ti-eye text-amber-400" aria-hidden="true" />
              </button>
            )}
          </div>
        )}

        {showLicense && v.business_license_url && (
          <PdfViewerModal
            url={v.business_license_url}
            title={`Leseni ya Udalali — ${v.user?.full_name ?? 'Dalali'}`}
