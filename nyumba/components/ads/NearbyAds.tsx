...
export default function NearbyAds({ region }: { region: string }) {
  ...
  useEffect(() => {
    if (!region) return
    fetch(`/api/v1/ads?type=nearby&region=${encodeURIComponent(region)}&limit=5`)
      .then(r => r.json())
      .then(d => setAds(d.ads ?? []))
      .catch(() => {})
  }, [region])
          ...
          return (
            ...
            >
              <div className="relative h-20 w-full bg-gray-100">
                {ad.image_url ? (
                  <Image src={ad.image_url} alt={ad.title} fill className="object-cover" sizes="144px" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-2xl">
                    {ad.advertiser?.logo_url
                      ? <Image src={ad.advertiser.logo_url} alt="" fill className="object-cover" sizes="144px" />
                      : '🏪'}
                  </div>
                )}
              </div>
              <div className="p-2">
                ...
                {ad.advertiser && (
                  <div className="text-[10px] text-gray-400 truncate">{ad.advertiser.business_category}</div>
                )}
              </div>
            </a>
          )
