...
type Ad = {
  ...
  cta_type: string; cta_value: string; target_category: string | null
  advertiser: { business_name: string; logo_url: string | null } | null
}

export default function SearchAd({ region }: { region?: string }) {
  const [ad, setAd] = useState<Ad | null>(null)

  useEffect(() => {
    const url = `/api/v1/ads?type=search${region ? `&region=${encodeURIComponent(region)}` : ''}&limit=1`
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {ad.advertiser && <div className="text-xs text-amber-600 font-medium">{ad.advertiser.business_name}</div>}
      </div>
      <div className="flex-shrink-0">
        {ad.cta_type === 'whatsapp' && <span className="text-xs bg-green-100 text-green-700 font-bold px-2 py-1 rounded-lg">WhatsApp</span>}
        {ad.cta_type === 'call'     && <span className="text-xs bg-blue-100 text-blue-700 font-bold px-2 py-1 rounded-lg">Piga Simu</span>}
        {ad.cta_type === 'website'  && <span className="text-xs bg-purple-100 text-purple-700 font-bold px-2 py-1 rounded-lg">Tovuti</span>}
      </div>
    </a>
  )
}
