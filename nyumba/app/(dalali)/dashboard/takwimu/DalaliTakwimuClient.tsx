...
function sourceLabel(src: string, t: (k: TKey) => string): string {
  if (src === 'direct')    return t('tak_src_direct')
  if (src === 'other')     return t('tak_src_other')
  const brands: Record<string, string> = {
    whatsapp: 'WhatsApp', facebook: 'Facebook', instagram: 'Instagram',
    google: 'Google', twitter: 'Twitter/X', tiktok: 'TikTok',
  }
  return brands[src] ?? src
}
...
export default function DalaliTakwimuClient() {
  ...
  const sourcesArr  = Object.entries(microsite?.sources ?? {}).sort((a, b) => b[1] - a[1]).slice(0, 6)
    ...
    <div className="min-h-screen bg-gray-50 pb-28">
      ...
      <div className="bg-gradient-to-br from-primary-700 via-primary-600 to-primary-500 px-4 pt-5 pb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <Link href="/dashboard" className="flex items-center gap-1 text-green-200 text-xs mb-1 hover:text-white transition">
              <i className="ti ti-arrow-left text-xs" aria-hidden="true" />
              Dashboard
            </Link>
            <h1 className="text-white font-bold text-lg leading-tight">{t('tak_title')}</h1>
            <p className="text-green-100 text-xs mt-0.5">{t('tak_subtitle')}</p>
          </div>
          <button
            onClick={load}
            className="w-9 h-9 bg-white/15 rounded-xl flex items-center justify-center text-white hover:bg-white/25 transition"
          >
        ...
        <div className="grid grid-cols-2 gap-2.5">
          <StatTile
            icon="ti-eye"
            label={t('tak_kpi_views_label')}
            value={fmtNum(microsite?.viewsMonth ?? 0)}
            sub={t('tak_kpi_views_sub')}
          />
          <StatTile
            icon="ti-users"
            label={t('tak_kpi_contacts_label')}
            value={fmtNum(account?.totalContacts ?? 0)}
            sub={t('tak_kpi_contacts_sub').replace('{{n}}', String(account?.thisMonthContacts ?? 0))}
          />
          <StatTile
            icon="ti-home"
            label={t('tak_kpi_listings_label')}
            value={`${account?.activeListings ?? 0}/${account?.totalListings ?? 0}`}
            sub={t('tak_kpi_listings_sub')}
          />
          <StatTile
            icon="ti-star"
            label={t('tak_kpi_rating_label')}
            value={ratingCount ? ratingAvg.toFixed(1) : '—'}
            sub={ratingCount ? t('tak_kpi_rating_sub').replace('{{n}}', String(ratingCount)) : t('tak_rating_none')}
          />
        </div>
      </div>
      ...
      <div className="flex border-b border-gray-200 bg-white sticky top-0 z-10 shadow-sm overflow-x-auto scrollbar-none">
        {([
          { key: 'overview',  labelKey: 'tak_tab_overview'  as TKey, icon: 'ti-layout-dashboard' },
          { key: 'microsite', labelKey: 'tak_tab_microsite' as TKey, icon: 'ti-world'            },
          { key: 'contacts',  labelKey: 'tak_tab_contacts'  as TKey, icon: 'ti-address-book'     },
          { key: 'maoni',     labelKey: 'tak_tab_maoni'     as TKey, icon: 'ti-star'             },
        ] as const).map(tab => (
          <button
            key={tab.key}
        ...
        {activeTab === 'overview' && (
          <>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
              ...
              <p className="text-xs text-gray-400 mb-4">{t('tak_chart_sub')}</p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
              ...
              {topListings.length === 0 ? (
                <p className="text-xs text-gray-400 text-center py-4">{t('tak_no_listings')}</p>
              ) : (
                  ...
                  {topListings.map((l, i) => (
                    ...
                    >
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        <p className="text-[10px] text-gray-400">{t('tak_listing_views')}</p>
                      </div>
                    </Link>
                  ))}
                </div>
            ...
            {/* Microsite preview button → Microsite tab */}
            {microsite && (
              <button
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="text-center">
                    ...
                    <p className="text-[10px] text-primary-500">{t('tak_today')}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-lg font-bold text-primary-700">{fmtNum(microsite.viewsWeek)}</p>
                    <p className="text-[10px] text-primary-500">{t('tak_this_week')}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-lg font-bold text-primary-700">{fmtNum(microsite.viewsMonth)}</p>
                    <p className="text-[10px] text-primary-500">{t('tak_this_month')}</p>
                  </div>
                </div>
          ...
          <>
            {!microsite ? (
              <div className="bg-white rounded-2xl border border-gray-100 p-6 text-center">
                ...
                <p className="text-sm font-medium text-gray-500">{t('tak_ms_no_data')}</p>
                {account?.username ? (
                  <Link href={`/agent/${account.username}`} target="_blank"
              ...
              <>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
                  <p className="text-sm font-semibold text-gray-800 mb-3 flex items-center gap-2">
                    ...
                    {t('tak_ms_views_title')}
                  </p>
                  <div className="grid grid-cols-2 gap-2">
                    <ViewTile label={t('tak_today')}          value={microsite.viewsToday}  accent="blue"    />
                    <ViewTile label={t('tak_this_week')}      value={microsite.viewsWeek}   accent="indigo"  />
                    <ViewTile label={t('tak_this_month_full')} value={microsite.viewsMonth}  accent="primary" />
                    <ViewTile label={t('tak_total')}          value={microsite.viewsTotal}  accent="green"   />
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <EngTile icon="ti-brand-whatsapp" color="text-green-600 bg-green-50"
                      label={t('tak_ms_wa_clicks')} value={microsite.whatsappClicks} />
                    <EngTile icon="ti-share" color="text-blue-600 bg-blue-50"
                      label={t('tak_ms_share')} value={microsite.shareCount} />
                    <EngTile icon="ti-home" color="text-primary-600 bg-primary-50"
                      label={t('tak_ms_listings')} value={microsite.clicks['listing_view'] ?? 0} />
                  </div>
                </div>
                        ...
                        return (
                          <div key={src}>
                            <div className="flex justify-between text-xs mb-1">
                              <span className="text-gray-700 font-medium">{sourceLabel(src, t)}</span>
                ...
                {microsite.viewsMonth > 0 && (
                  <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-100 rounded-2xl p-4">
                    ...
                    <p className="text-xs text-amber-600 mt-0.5">
                      {t('tak_ms_conversion_sub')
                        .replace('{{clicks}}', String(microsite.whatsappClicks))
                        .replace('{{views}}', String(microsite.viewsMonth))}
                    </p>
                  </div>
                )}
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="text-center">
                      ...
                      <p className="text-[10px] text-gray-400 mt-0.5">{t('tak_ms_alltime_views')}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-xl font-bold text-gray-800">{fmtNum(microsite.whatsappClicks)}</p>
                      <p className="text-[10px] text-gray-400 mt-0.5">{t('tak_ms_alltime_wa')}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-xl font-bold text-gray-800">{fmtNum(account?.totalContacts ?? 0)}</p>
                      <p className="text-[10px] text-gray-400 mt-0.5">{t('tak_ms_alltime_contacts')}</p>
                    </div>
                  </div>
                </div>
                ...
                {account?.username && (
                  <Link
                    href={`/agent/${account.username}`}
                    target="_blank"
                    className="flex items-center justify-between bg-white border border-gray-100 rounded-2xl px-4 py-3 shadow-sm hover:bg-gray-50 transition"
                  >
                    <div>
                      <p className="text-xs font-semibold text-gray-800">{t('tak_ms_profile')}</p>
                      <p className="text-[11px] text-gray-400 font-mono">nyumbafasta.co/agent/{account.username}</p>
                    </div>
                    <i className="ti ti-external-link text-gray-400" aria-hidden="true" />
                  </Link>
                )}
              </>
            )}
          ...
          <>
            <div className="flex items-center justify-between">
              <p className="text-xs text-gray-500">
                {contacts.length > 0
                  ? t('tak_contacts_title').replace('{{n}}', String(contacts.length))
                  : t('tak_contacts_label')}
              </p>
              {account && account.totalContacts > 30 && (
                <span className="text-xs text-amber-600 font-medium">{t('tak_showing_30')}</span>
              )}
            </div>
            ...
            {contacts.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
                ...
                <p className="text-gray-500 font-medium text-sm">{t('tak_contacts_empty')}</p>
                <p className="text-gray-400 text-xs mt-1">
                  {t('tak_contacts_empty_sub')}
                </p>
              </div>
            ) : (
              <div className="space-y-2">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-gray-900 text-sm">
                          {c.client?.full_name ?? t('tak_client_default')}
                        </p>
          ...
          <>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
                <div className="flex items-center gap-4">
                  <div className="text-center flex-shrink-0">
                    ...
                    <p className="text-[11px] text-gray-400 mt-1">{t('tak_maoni_count').replace('{{n}}', String(ratingCount))}</p>
                  </div>
                ...
                {fiveStar > 0 && (
                  <div className="mt-3 bg-amber-50 rounded-xl px-3 py-2 flex items-center gap-2">
                    ...
                    <p className="text-xs text-amber-700 font-medium">
                      {t('tak_five_star').replace('{{n}}', String(fiveStar)).replace('{{total}}', String(ratingCount))}
                    </p>
                  </div>
                )}
              </div>
            ...
            {reviews.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
                ...
                <p className="text-gray-500 font-medium text-sm">{t('tak_maoni_empty')}</p>
                <p className="text-gray-400 text-xs mt-1 leading-relaxed max-w-[200px] mx-auto">
                  {t('tak_maoni_empty_sub')}
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-xs font-semibold text-gray-800">
                            {r.reviewer?.full_name ?? t('tak_client_default')}
                            {r.is_verified && (
                              <i className="ti ti-circle-check text-primary-500 text-[11px] ml-1" aria-hidden="true" />
                            )}
                          </p>
                          <p className="text-[10px] text-gray-400">{timeAgo(r.created_at, t)}</p>
                        </div>
                      </div>
                    ...
                    {r.response ? (
                      ...
                      <div className="mt-2 space-y-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          >
                            {replyLoading ? t('tak_sending') : t('tak_send_reply')}
                          </button>
                          <button
