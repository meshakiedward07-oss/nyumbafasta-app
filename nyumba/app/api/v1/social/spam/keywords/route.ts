...
export async function GET() {
  try {
    ...
    console.error('[GET app/api/v1/social/spam/keywords]', msg)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err)
    console.error('[POST app/api/v1/social/spam/keywords]', msg)
    return NextResponse.json({ error: 'Hitilafu ya seva' }, { status: 500 })
  }
}
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err)
    console.error('[PATCH app/api/v1/social/spam/keywords]', msg)
    return NextResponse.json({ error: 'Hitilafu ya seva' }, { status: 500 })
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err)
    console.error('[DELETE app/api/v1/social/spam/keywords]', msg)
    return NextResponse.json({ error: 'Hitilafu ya seva' }, { status: 500 })
  }
}
