...
export async function GET() {
  try {
    ...
    const { data, error } = await admin
      .from('listings')
      .select('id, title, type, status, price_monthly, district, region, images, view_count, lead_count, share_count, created_at, is_boosted, boosted_until, expires_at, listing_unit_type, total_capacity, current_occupancy')
