    ...
    <div className="flex flex-col h-full max-w-3xl mx-auto overflow-y-auto">
      ...
      <div className="p-4 lg:p-5 border-b border-gray-100 bg-white flex-shrink-0">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-gray-900">{t('pr_taarifa_title')}</h1>
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            {listings.length > 0 && (
              ...
              >
                <option value="">{t('pr_taarifa_all_properties')}</option>
            ...
            {data && (
              <button
                ...
                title={t('pr_taarifa_download_excel')}
              >
                ⬇
              </button>
            )}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center gap-4 px-4">
            ...
            <h2 className="text-lg font-bold text-gray-900">{t('pr_taarifa_locked_heading')}</h2>
            <p className="text-sm text-gray-500 max-w-xs">
              {t('pr_taarifa_locked_long_desc')}
            </p>
            <a href="/property/usajili"
              ...
              {t('pr_taarifa_locked_view_plans')}
            </a>
          </div>
        ) : error ? (
          <div className="text-center py-12">
            ...
            <button onClick={() => orgId && loadReport(orgId, month)}
              className="text-primary-600 text-sm mt-2 underline">{t('pr_btn_try_again')}</button>
          </div>
        ) : data ? (
          <>
            {/* ── Income Summary ─────────────────────────────────────────── */}
            <div className="bg-white border border-gray-100 rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="font-semibold text-gray-900 text-sm">{t('pr_taarifa_income_section')} — {monthLabel}</h2>
              </div>

              {/* Collection rate bar */}
              <div className="mb-4">
                <div className="flex justify-between items-center mb-1.5">
                  <span className="text-xs text-gray-500">{t('pr_taarifa_income_heading')}</span>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { label: t('pr_taarifa_due'),          value: data.income.total_due,       color: 'text-gray-800'  },
                  { label: t('pr_taarifa_collected'),    value: data.income.total_collected, color: 'text-green-600' },
                  { label: t('pr_taarifa_pending_label'),value: data.income.total_pending,   color: 'text-amber-600' },
                  { label: t('pr_taarifa_overdue_label'),value: data.income.total_overdue,   color: 'text-red-600'   },
                ].map(card => (
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-semibold text-gray-900 text-sm">{t('pr_taarifa_occupancy_heading')}</h2>
                </div>
              ...
              <div className="flex gap-3 text-xs flex-wrap">
                <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-primary-400 inline-block" />{data.occupancy.occupied} {t('pr_taarifa_occupied')}</span>
                <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-gray-200 inline-block" />{data.occupancy.vacant} {t('pr_taarifa_vacant')}</span>
                {data.occupancy.maintenance > 0 && (
                  <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-amber-300 inline-block" />{data.occupancy.maintenance} {t('pr_nav_maintenance')}</span>
                )}
              </div>

              {data.occupancy.monthly_income > 0 && (
                <div className="mt-3 pt-3 border-t border-gray-50">
                  <p className="text-xs text-gray-400">{t('pr_taarifa_monthly_total')}</p>
            ...
            {data.monthly_income.length > 0 && (
              <div className="bg-white border border-gray-100 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="font-semibold text-gray-900 text-sm">{t('pr_taarifa_chart_heading')}</h2>
                </div>
                ...
                <div className="flex gap-4 mt-2 text-[10px] text-gray-400">
                  <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-gray-200 inline-block" />{t('pr_taarifa_chart_due')}</span>
                  <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-primary-400 inline-block" />{t('pr_taarifa_collected')}</span>
                </div>
              </div>
            )}
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="font-semibold text-gray-900 text-sm">{t('pr_taarifa_maint_heading')}</h2>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <div className="bg-blue-50 rounded-xl p-3 text-center">
                  <p className="text-xl font-bold text-blue-600">{data.maintenance_summary.open}</p>
                  <p className="text-[10px] text-blue-500 font-medium">{t('pr_maint_tab_open')}</p>
                </div>
                <div className="bg-amber-50 rounded-xl p-3 text-center">
                  <p className="text-xl font-bold text-amber-600">{data.maintenance_summary.in_progress}</p>
                  <p className="text-[10px] text-amber-500 font-medium">{t('pr_taarifa_maint_in_progress')}</p>
                </div>
              ...
              {data.maintenance_summary.total_actual_cost > 0 && (
                <div className="mt-3 pt-3 border-t border-gray-50">
                  <p className="text-xs text-gray-400">{t('pr_taarifa_maint_this_month')}</p>
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="font-semibold text-gray-900 text-sm">{t('pr_matumizi_title')}</h2>
                  </div>
                  <a href="/property/matumizi" className="text-xs text-primary-500 font-medium">{t('pr_taarifa_manage_link')} →</a>
                </div>

                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div className="bg-red-50 rounded-xl p-3 text-center">
                    <p className="text-xs text-gray-500 mb-1">{t('pr_taarifa_expense_total')}</p>
                  ...
                  <div className={`rounded-xl p-3 text-center ${data.profit_loss.net >= 0 ? 'bg-green-50' : 'bg-red-50'}`}>
                    <p className="text-xs text-gray-500 mb-1">{data.profit_loss.net >= 0 ? t('pr_taarifa_profit') : t('pr_taarifa_loss')}</p>
            ...
            {(data.leases_ending_soon as unknown as Array<{
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h2 className="font-semibold text-gray-900 text-sm">{t('pr_taarifa_leases_ending')}</h2>
                  <span className="ml-auto text-[10px] bg-orange-50 text-orange-600 px-2 py-0.5 rounded-full font-medium">
                    {t('pr_taarifa_days_60')}
                  </span>
                </div>
            ...
            {(data.overdue_payments as unknown as Array<{
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h2 className="font-semibold text-gray-900 text-sm">{t('pr_taarifa_overdue_payments')}</h2>
                    ...
                    <div key={p.id} className="flex items-center justify-between bg-red-50/50 rounded-xl p-3">
                      ...
                      <div className="flex-shrink-0 text-right ml-2">
                        ...
                        {p.lease?.tenant?.phone && (
                          <a href={`tel:${p.lease.tenant.phone}`} className="text-[10px] text-primary-600 font-medium">{t('pr_taarifa_call')}</a>
                        )}
                      </div>
                    </div>
                  ))}
            ...
            {data.occupancy.total_units === 0 && (
              <div className="text-center py-12 px-6">
                ...
                <p className="text-gray-500 font-medium mt-3">{t('pr_taarifa_no_data_title')}</p>
                <p className="text-sm text-gray-400 mt-1">{t('pr_taarifa_no_data_desc')}</p>
              </div>
            )}
          </>
        ) : null}
