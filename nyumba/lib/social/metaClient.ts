...
// FB uses FACEBOOK_PAGE_ACCESS_TOKEN (dedicated permanent System User token) as primary.
// Falls back to INSTAGRAM_ACCESS_TOKEN for setups that use one token for both platforms.
const fbToken   = () => process.env.FACEBOOK_PAGE_ACCESS_TOKEN ?? process.env.INSTAGRAM_ACCESS_TOKEN ?? process.env.FACEBOOK_ACCESS_TOKEN ?? ''
